...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Prep.Func.PreparingToSend
{
    ...
    public class SyncTeamMembersActivity
    {
        ...
        public async Task RunAsync(
            [ActivityTrigger](string notificationId, string teamId, string exclusionL) input,
            ILogger log)
        {
            ...
            try
            {
                ...
                var recipients = await this.GetRecipientsAsync(notificationId, userEntities, exclusionL);
        ...
        /// <param name="exclusionL">Exclusion.</param>
        /// <returns>List of recipients.</returns>
        private async Task<IEnumerable<SentNotificationDataEntity>> GetRecipientsAsync(string notificationId, IEnumerable<UserDataEntity> users, string exclusionL)
        {
            string[] arrString = exclusionL.Split(";");
            ...
            await Task.WhenAll(users.ForEachAsync(maxParallelism, async user =>
            {
                ...
                if (arrString.Contains(user.Upn, StringComparer.OrdinalIgnoreCase))
                {
                    return;
                }
